...
# parseur: 
```{r}
Sample = read.table("name",sep=";",header=FALSE) # This command returns an object of the class data.frame
tab_X_1 = as.matrix(Sample) # For our purpose it is better to convert it into a matrix
n = dim(tab_X_1)[1] # Number of rows
p = dim(tab_X_1)[2] # Number of columns
X1_char = tab_X_1[2:n,p] 

Sample_2 = read.table("name2",sep=";",header=FALSE) # This command returns an object of the class data.frame
tab_X_2 = as.matrix(Sample_2) # For our purpose it is better to convert it into a matrix
number_of_rows = dim(tab_X_2)[1] # Number of rows
number_of_columns = dim(tab_X_2)[2] # Number of columns
X2_char = tab_X_2[2:number_of_rows,number_of_columns] 


X_1=c()
for (k in 1:length(X1_char)){
  X_1<-c(X_1,as.numeric(X1_char[k]))
}
X_2=c()
for (k in 1:length(X2_char)){
  X_2<-c(X_2,as.numeric(X2_char[k]))
}
...
for (j in 1:p){
  gamma_j=0
  for (t in 1+j:n ){
    gamma_j=gamma_j + (Y[t]-Y_barre)(Y[t-j]-Y_barre)
  }
  gamma_j=gamma_j/(n-j)
  Qp=Qp+gamma_j**2
}
...
l'hypothèse alternantive est: $ H_1 = \{ |Q_p| \geq q_{chisq}(1-\alpha/2) \}$


```{r}
for (alpha in c(0.1,0.05,0.01)){
  quantile_Qp = qchisq(1-alpha/2,df=p)
  p_value= 2*pnorm(-abs(quantile_Qp),df=p)
  print(paste0(" alpha = ", alpha))
  print(paste0(" Qp = ",zeta))
  print(paste0(" Quantile = ",quantile))
