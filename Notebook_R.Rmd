...
on a :
$$ \operatorname{E}[X_t]=\theta^t X_0 + c\sum_{i=0}^{t-1}\theta^i $$ 
$$ \operatorname{Var}[X_t]= \sum_{i=0}^{t}\theta^{2i}\sigma^2 $$ 
$$\operatorname{Cov}[X_t,X_{t-j}]= \theta^{j}\sum_{i=0}^{t-j}\theta^{2i}\sigma^2 $$ 

donc on obtient: 

$$ \rho_j = \operatorname{Corr}[X_t,X_{t-j}]\equiv \frac{\operatorname{Cov}[X_t,X_{t-j}]}{\sqrt{\operatorname{Var}(X_t)\operatorname{Var}(X_{t-j})}}=\theta^j \sqrt{\frac{1 - \theta^{2(t-j) + 2}}{1 - \theta^{2t + 2}}} $$ 
...
$$ \rho_j =\operatorname{Corr}[X_t,X_{t-j}]= \theta^j$$
...
var_cov = function(t,theta,sigma,bk){
  var = sum(bk)+ 2*( (1-theta^t)/(1-theta) -t ) * sigma^2/((1-theta^(-1))*(1-theta)) - 2*(theta*sigma)^2* ( (1-theta^t)/(1-theta) -  (1-theta^(2*t))/(1-theta^2) )/(1-theta)^2
  return(var)
}
...
sums = function(Sample, theta, t_max){
  ...
  bk= c(sigma**2)
  for (j in (1:t_max)){
    sum[j] = mean(Sample[,j])*sqrt(n/var_cov(j,theta,sigma,bk))
    
    ak = c(ak,theta*ak[length(ak)] + c)
    bk = c(bk,theta**2 * bk[length(bk)]+ sigma^2)
  }
  sum[1]=0
  return(sum)
}
...
La limite du test précédent est qu'on ne considère seulement un nombre fini d'autocorrélations. On va donc utiliser la densité spectrale (qui contient toutes les covariances).

On définit la densité spectrale $f(\lambda)$ par

$$ \gamma_k = \int_{\Pi}^{}f(\lambda)exp(ik\lambda)d\lambda  \\
k = 0,1,2,... \\
\Pi = [-\pi, \pi]$$

On définit également le périodigramme par 

$$ I(\lambda) = \lvert w(\lambda) \rvert ^2 \\
\text{où } w(\lambda) = n^{-1/2} \sum_{t=1}^{n} x_t exp(it\lambda)$$

L'intégrale de la densité spectrale est linéaire en $\lambda$ sous l'hypothèse de MDH.

On a alors l'hypothèse nulle
...
On introduit donc le périodigramme cumulatif standardisé

$$ Z_n(\lambda) = \sqrt{T} ( \frac{\sum_{j=1}^{[\lambda T  \pi]} I(\lambda_j)}{\sum_{j=1}^{T} I(\lambda_j)} - \frac{\lambda}{\pi} ) \\
\text{où } \lambda_j = \frac{2\pi j}{n} \\
j = 1,2,..., n/2$$

A partir de $Z_n$, on peut en déduire le test de Kolmogorov Smirnov
...
On introduit la statistique de Deo (qui peut être vue comme une version continue de la statistique $Q_p^*$). Elle s'écrit
