...
Qpstar = function(Sample, p) {
  ...
  for (i in 1:n) {
    ...
    for (j in 1:p){
      ...
      tau_j = sum((Sample[i,(1+j):d]-Y_barre)^2*(Sample[i,1:(d-j)]-Y_barre)^2) /(gamma_0^2)
...
theorie_vs_pratique = function(p){
  ...
  for (thet in theta){
    ...
    for(j in (1:p)){
      Qp_theo_j = Qp_theo_j + thet^(2*j)
    }
    Qp_theo=c(Qp_theo,Qp_theo_j)
    
  }
  print(Qp_theo)
  plot(theta, Qps,col = "blue", type = "l")
...
graph_power = function(n){
  ...
  lines(theta, pwr3, col="green") # la courbe est a zero on ne la voit pas il faut trouver l'erreur 
